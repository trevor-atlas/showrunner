...
async function waitForAgentStream(baseUrl: string, runId: string): Promise<void> {
  await waitFor(async () => {
    const { json } = await api(baseUrl, "GET", `/runs/${runId}/raw?n=1`);
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const before = (await api(baseUrl, "GET", `/runs/${runId}`)).json as {
        sessions: { pid: number }[];
        event_count: number;
      };
      ...
      const interruptedDetail = (await api(baseUrl, "GET", `/runs/${runId}`)).json as {
...
test("GET /status reports health, pool utilization, and run status counts", async () => {
  ...
  try {
    ...
    const s1 = (await api(baseUrl, "GET", "/status")).json as {
      pool: { running: string[] };
      runs: Record<string, number>;
    };
    ...
    await waitForStatus(baseUrl, runId, "success");
    const s2 = (await api(baseUrl, "GET", "/status")).json as {
      pool: { running: string[] };
      runs: Record<string, number>;
    };
...
test("graceful shutdown stops recorded children and removes the pidfile", async () => {
  ...
  try {
    ...
    const detail = (await api(baseUrl, "GET", `/runs/${runId}`)).json as { sessions: { pid: number }[] };
...
test("kill-9 does not tear the events log: ids are contiguous and strictly ascending", async () => {
  ...
  try {
    ...
    const runId = (sub.json as { run_id: string }).run_id;
    await waitForAgentStream(baseUrl, runId);
    const before = (await api(baseUrl, "GET", `/runs/${runId}/events?cursor=0&limit=500`)).json as {
      events: { id: number }[];
    };
