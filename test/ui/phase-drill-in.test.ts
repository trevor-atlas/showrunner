...
import { insertPhase, insertRun, openDb, updateRun } from "../../src/daemon/db.ts";
...
describe("phase drill-in (T11)", () => {
  ...
  it("renders the drill-in cards against an in-process daemon (the old daemon-down shell is impossible)", async () => {
    const dir = tmpDir("drillin-in-process");
    const restore = setDataDir(dir);
    let daemon: DaemonHandle | null = null;
    try {
      // seed a run + phase directly so the daemon has rows to look up
      const runId = "33333333-aaaa-4aaa-8aaa-aaaaaaaaaaaa";
      const db = openDb(dbPathFor(dir));
      insertRun(db, {
        id: runId,
        blueprint: "seeded-demo",
        status: "success",
        cwd: "/tmp/x",
        needs_review: 0,
        started_at: new Date().toISOString(),
      ...
      const res = await fetchDrillIn(runId, "build");
      expect(res.status).toBe(200);
      expect(res.html).toContain("seeded-demo");
      expect(res.html).toContain("CONFIG");
      expect(res.html).toContain("ENVELOPE");
      expect(res.html).toContain("GATES");
