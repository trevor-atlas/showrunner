...
Send the user's `{prompt}` to a single agent through Showrunner. The starter kit's `prompt` blueprint runs exactly one phase with one agent — by default the **planner**; edit `src/starter-kit/blueprints/prompt.ts` (or copy it) to swap in `builder`, `scout`, `reviewer`, `documenter`, or `ship`.
...
The CLI takes a blueprint **module path**; `prompt` is the starter-kit name and resolves to `src/starter-kit/blueprints/prompt.ts` (see that package's README for the name→path map). If your CLI does not accept bare names yet, use the path form:
