...
The CLI takes a blueprint **module path**; `build_test` is the starter-kit name and resolves to `packages/starter-kit/src/blueprints/build_test.ts` (see that package's README for the name→path map). If your CLI does not accept bare names yet, use the path form:

```bash
showrunner run packages/starter-kit/src/blueprints/build_test.ts --prompt "<what to build>"
```
