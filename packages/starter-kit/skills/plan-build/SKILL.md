...
The CLI takes a blueprint **module path**; `plan_build` is the starter-kit name and resolves to `packages/starter-kit/src/blueprints/plan_build.ts` (see that package's README for the name→path map). If your CLI does not accept bare names yet, use the path form:

```bash
showrunner run packages/starter-kit/src/blueprints/plan_build.ts --prompt "<the work>"
```
