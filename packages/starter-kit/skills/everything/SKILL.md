...
The CLI takes a blueprint **module path**; `everything` is the starter-kit name and resolves to `packages/starter-kit/src/blueprints/everything.ts` (see that package's README for the name→path map). If your CLI does not accept bare names yet, use the path form:

```bash
showrunner run packages/starter-kit/src/blueprints/everything.ts --prompt "<the work>"
```
