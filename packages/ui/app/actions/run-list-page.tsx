 ...
 * through the server-side client; the browser sees only rendered HTML. Rows
 * link to the run-detail route (a stub 404 until the detail ticket).
 */

export interface RunListPageProps {
  runs: RunListItem[];
  daemonDown: boolean;
  /** the daemon address for the down banner ("expected at …") */
...
export function RunListPage(handle: Handle<RunListPageProps>) {
  return () => {
    const { runs, daemonDown, daemonAddress, filter, statuses } = handle.props;
    const visible = daemonDown ? [] : filterRuns(runs, filter);
