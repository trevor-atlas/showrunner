...
import { dbPathFor, type EventType } from "@showrunner/core";
import { DaemonClient } from "../../daemon/src/client.ts";
import { startDaemon, type DaemonHandle } from "../../daemon/src/daemon.ts";
import { insertEvent, insertPhase, insertRun, openDb } from "../../daemon/src/db.ts";
...
describe("run detail (T10a) — server-side daemon data + the cursor proxy", () => {
  ...
  it("renders the shell with the daemon-down banner instead of 500ing (§16.10)", async () => {
    const dir = tmpDir("detail-down");
    const restore = setDataDir(dir);
    try {
      const res = await fetchDetail("11111111-aaaa-4aaa-8aaa-aaaaaaaaaaaa");
