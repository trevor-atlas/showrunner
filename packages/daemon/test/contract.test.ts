...
import { CURSOR_SQL } from "../src/db.ts";
import {
  ApiError,
  DaemonClient,
  insertPhase,
 ...
 * run_submitted-at-acceptance fixes (F2), and the typed client over BOTH
 * transports (unix socket + the SHOWRUNNER_DAEMON_URL http override).
 */
...
async function runEvents(socketPath: string, runId: string): Promise<{ id: number; type: string; data: Record<string, unknown> }[]> {
  const { json } = await api(socketPath, "GET", `/runs/${runId}/events?cursor=0&limit=500`);
...
test("§13.1 read surface: detail, envelopes, gates, spend, raw tail, list — and the 404 semantics", async () => {
  ...
  try {
    ...
    const sub = await api(socketPath, "POST", "/runs", { blueprint: DEMO, cwd, delayMs: 0 });
    expect(sub.status).toBe(201);
    const { run_id } = sub.json as { run_id: string };
    await waitForStatus(socketPath, run_id, "success");

    // GET /runs — list with id/blueprint/status/started/ended/spend + queue position
    const list = ((await api(socketPath, "GET", "/runs")).json as { runs: Record<string, unknown>[] }).runs;
    ...
    const detail = ((await api(socketPath, "GET", `/runs/${run_id}`)).json as {
    ...
    const planEnv = ((await api(socketPath, "GET", `/runs/${run_id}/phases/plan/envelopes`)).json as {
      run_id: string;
      phase: string;
      phase_id: string;
    ...
    const planGates = ((await api(socketPath, "GET", `/runs/${run_id}/phases/plan/gates`)).json as {
    ...
    const spend = ((await api(socketPath, "GET", `/runs/${run_id}/spend`)).json as {
      run_id: string;
      spend_usd: number;
    ...
    const raw = ((await api(socketPath, "GET", `/runs/${run_id}/raw?lines=3`)).json as {
      run_id: string;
      raw: string;
      line_count: number;
      truncated: boolean;
    ...
    const full = ((await api(socketPath, "GET", `/runs/${run_id}/raw?n=99999`)).json as {
      truncated: boolean;
      line_count: number;
    });
    ...
    for (const [m, p] of notFound) {
      const res = await api(socketPath, m, p);
...
test("§4.3 cursor contract: limit honored (500 cap, default), next_cursor advances, idempotent at the tail", async () => {
  ...
  try {
    ...
    for (const qs of ["", "&limit=1000"]) {
      const page = ((await api(socketPath, "GET", `/runs/${runId}/events?cursor=0${qs}`)).json as {
        events: { id: number }[];
        next_cursor: number;
      });
    ...
    const first = ((await api(socketPath, "GET", `/runs/${runId}/events?cursor=0&limit=500`)).json as {
      events: { id: number }[];
      next_cursor: number;
    });
    const second = ((await api(socketPath, "GET", `/runs/${runId}/events?cursor=${first.next_cursor}&limit=500`)).json as {
      events: { id: number }[];
      next_cursor: number;
    });
    ...
    const tail = ((await api(socketPath, "GET", `/runs/${runId}/events?cursor=${second.next_cursor}`)).json as {
      events: unknown[];
      next_cursor: number;
    });
    ...
    const ids: number[] = [];
    for (let i = 0; i < 20; i++) {
      const p = ((await api(socketPath, "GET", `/runs/${runId}/events?cursor=${cursor}&limit=250`)).json as {
        events: { id: number }[];
        next_cursor: number;
      });
...
test("F2: run_submitted fires at acceptance (a queued run has it before it drives); queue position surfaces on POST /runs and GET /runs", async () => {
  ...
  try {
    ...
    const a = await api(socketPath, "POST", "/runs", { blueprint: APPROVAL, cwd, delayMs: 0 });
    ...
    await waitForStatus(socketPath, aId, "paused");

    // B: queued behind A — the submit response carries its queue position
    const b = await api(socketPath, "POST", "/runs", { blueprint: DEMO, cwd, delayMs: 0, args: ["--go", "fast"] });
    ...
    const list = ((await api(socketPath, "GET", "/runs")).json as {
    ...
    const bEvents = await runEvents(socketPath, bId);
    ...
    const ok = await api(socketPath, "POST", `/runs/${aId}/approve`);
    expect(ok.status).toBe(200);
    await waitForStatus(socketPath, bId, "success");
    const bAll = await runEvents(socketPath, bId);
    ...
    const after = ((await api(socketPath, "GET", "/runs")).json as {
      runs: { id: string; queue_position: number | null }[];
    }).runs;
    ...
    const c = await api(socketPath, "POST", "/runs", { blueprint: APPROVAL, cwd, delayMs: 0 });
    ...
    await waitForStatus(socketPath, cId, "paused");
    const d = await api(socketPath, "POST", "/runs", { blueprint: HAPPY, cwd, delayMs: 0 });
    const dId = (d.json as { run_id: string }).run_id;
    expect((d.json as { queue_position: number | null }).queue_position).toBe(1);
    const e = await api(socketPath, "POST", "/runs", { blueprint: HAPPY, cwd, delayMs: 0 });
    ...
    await api(socketPath, "POST", `/runs/${cId}/fail`);
    await waitForStatus(socketPath, dId, "success");
    await waitForStatus(socketPath, eId, "success");
  } finally {
    await daemon?.close();
    cleanupDir(dir);
    rmSync(cwd, { recursive: true, force: true });
...
test("§13.2 control surface: approve + audited gate override + restart-fresh + fail, with human_action events and guardrails", async () => {
  ...
  try {
    ...
    const ap = await api(socketPath, "POST", "/runs", { blueprint: APPROVAL, cwd, delayMs: 0 });
    const apId = (ap.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, apId, "paused");
    const viewer = ((await api(socketPath, "GET", `/runs/${apId}/pause`)).json as {
      paused: boolean;
      kind: string;
      phase: string;
      actions: string[];
    ...
    const approved = await api(socketPath, "POST", `/runs/${apId}/approve`, { by: "operator" });
    expect(approved.status).toBe(200);
    await waitForStatus(socketPath, apId, "success");
    const apEvents = await runEvents(socketPath, apId);
    ...
    const pu = await api(socketPath, "POST", "/runs", { blueprint: PAUSE, cwd, delayMs: 0 });
    const puId = (pu.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, puId, "paused");
    const budgetView = ((await api(socketPath, "GET", `/runs/${puId}/pause`)).json as {
      kind: string;
      phase: string;
      actions: string[];
    });
    ...
    const badGate = await api(socketPath, "POST", `/runs/${puId}/phases/build/override`, { gate: "nope", reason: "why" });
    expect(badGate.status).toBe(409);
    const badApprove = await api(socketPath, "POST", `/runs/${puId}/approve`);
    expect(badApprove.status).toBe(409);
    const wrongPhase = await api(socketPath, "POST", `/runs/${puId}/phases/other/override`, { gate: "alwaysFail", reason: "why" });
    expect(wrongPhase.status).toBe(409);

    // the audited override: who + why recorded, original gate_results row kept
    const over = await api(socketPath, "POST", `/runs/${puId}/phases/build/override`, {
      gate: "alwaysFail",
      reason: "human inspection says it is fine",
      by: "operator",
    });
    expect(over.status).toBe(200);
    await waitForStatus(socketPath, puId, "success");
    const gates = ((await api(socketPath, "GET", `/runs/${puId}/phases/build/gates`)).json as {
    ...
    const puEvents = await runEvents(socketPath, puId);
    ...
    const rf = await api(socketPath, "POST", "/runs", { blueprint: PAUSE, cwd, delayMs: 0 });
    const rfId = (rf.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, rfId, "paused");
    const restarted = await api(socketPath, "POST", `/runs/${rfId}/phases/build/restart-fresh`, { by: "human" });
    expect(restarted.status).toBe(200);
    // the new visit fails its gate again → the run re-pauses (visits=2)
    await waitForStatus(socketPath, rfId, "paused");
    const rfDetail = ((await api(socketPath, "GET", `/runs/${rfId}`)).json as {
      phases: { name: string; visits: number }[];
    });
    expect(rfDetail.phases[0]!.visits).toBe(2);
    const rfEvents = await runEvents(socketPath, rfId);
    expect(rfEvents.some((e) => e.type === "human_action" && e.data.action === "restart")).toBe(true);

    const failed = await api(socketPath, "POST", `/runs/${rfId}/fail`, { by: "human" });
    expect(failed.status).toBe(200);
    await waitForStatus(socketPath, rfId, "failed");
    const failEvents = await runEvents(socketPath, rfId);
...
test("§13.2 session steer delivers between turns; resume and fail answer 409 outside their contract", async () => {
  ...
  try {
    ...
    const ghost = await api(socketPath, "POST", "/sessions/nope/steer", { message: "hi" });
    expect(ghost.status).toBe(409);
    // empty message → 409
    const empty = await api(socketPath, "POST", "/sessions/nope/steer", { message: "   " });
    expect(empty.status).toBe(409);

    // live-session steer: a slow blueprint run holds its session long enough
    const sub = await api(socketPath, "POST", "/runs", { blueprint: DEMO, cwd, delayMs: 120 });
    ...
    const steered = await api(socketPath, "POST", `/sessions/${piSessionId}/steer`, { message: "check the tests too", by: "reviewer" });
    expect(steered.status).toBe(200);
    expect((steered.json as { ok: boolean }).ok).toBe(true);
    await waitForStatus(socketPath, runId, "success");
    const evts = await runEvents(socketPath, runId);
    ...
    const resumeSuccess = await api(socketPath, "POST", `/runs/${runId}/resume`);
    expect(resumeSuccess.status).toBe(409);
    const resumeGhost = await api(socketPath, "POST", "/runs/ghost/resume");
    expect(resumeGhost.status).toBe(404);

    // fail guardrail: a finished run is already terminal → 409
    const failFinished = await api(socketPath, "POST", `/runs/${runId}/fail`);
...
test("§13.2 resume over HTTP: an interrupted run relaunches from its recorded phase and succeeds", async () => {
  ...
  try {
    ...
    const ghost = await api(socketPath, "POST", "/runs/ghost/resume");
    expect(ghost.status).toBe(404);

    const resumed = await api(socketPath, "POST", `/runs/${runId}/resume`, { by: "operator" });
...
test("§14 hooks fire with ctx.shell() in the run cwd (onPhaseStart AND onPhaseEnd)", async () => {
  ...
  try {
    daemon = startDaemon({ dataDir: dir });
    const socketPath = daemon.socketPath!;
    const sub = await api(socketPath, "POST", "/runs", { blueprint: HOOK, cwd, delayMs: 0 });
    const runId = (sub.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, runId, "success");
...
test("§14 a throwing onPhaseStart audits + parks the run at the hook_failed menu (never dies silently)", async () => {
  ...
  try {
    daemon = startDaemon({ dataDir: dir });
    const socketPath = daemon.socketPath!;
    const sub = await api(socketPath, "POST", "/runs", { blueprint: HOOK_START_FAIL, cwd, delayMs: 0 });
    const runId = (sub.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, runId, "paused");

    const viewer = ((await api(socketPath, "GET", `/runs/${runId}/pause`)).json as {
      kind: string;
      phase: string;
      reason: string;
      actions: string[];
    ...
    const failed = await api(socketPath, "POST", `/runs/${runId}/fail`);
    expect(failed.status).toBe(200);
    await waitForStatus(socketPath, runId, "failed");
  } finally {
    await daemon?.close();
    cleanupDir(dir);
    rmSync(cwd, { recursive: true, force: true });
...
test("§14 a throwing onPhaseEnd records the phase FAILED + audits, then parks at the hook_failed menu", async () => {
  ...
  try {
    daemon = startDaemon({ dataDir: dir });
    const socketPath = daemon.socketPath!;
    const sub = await api(socketPath, "POST", "/runs", { blueprint: HOOK_END_FAIL, cwd, delayMs: 0 });
    const runId = (sub.json as { run_id: string }).run_id;
    await waitForStatus(socketPath, runId, "paused");

    const viewer = ((await api(socketPath, "GET", `/runs/${runId}/pause`)).json as { kind: string; reason: string });
    expect(viewer.kind).toBe("hook_failed");
    ...
    await api(socketPath, "POST", `/runs/${runId}/fail`);
    await waitForStatus(socketPath, runId, "failed");
  } finally {
    await daemon?.close();
    cleanupDir(dir);
...
test("§13.3 submit errors are clean 400s: missing module, bad args, no fixture/blueprint", async () => {
  ...
  try {
    ...
    const missing = await api(socketPath, "POST", "/runs", { blueprint: "/nonexistent/blueprint.ts" });
    expect(missing.status).toBe(400);
    expect(typeof (missing.json as { error: string }).error).toBe("string");

    const badArgs = await api(socketPath, "POST", "/runs", { blueprint: HAPPY, args: "nope" });
    expect(badArgs.status).toBe(400);

    const neither = await api(socketPath, "POST", "/runs", {});
...
// ── the typed client: unix socket AND the http SHOWRUNNER_DAEMON_URL override ─

test("typed client works over the unix socket and the http override; ApiError carries the status", async () => {
  const dir = tmpDataDir("contract-client");
  const cwd = scratchCwd("contract-client-cwd");
  let unixDaemon: DaemonHandle | null = null;
  let httpDaemon: DaemonHandle | null = null;
  const savedUrl = process.env.SHOWRUNNER_DAEMON_URL;
  try {
    // ── unix transport ──
    unixDaemon = startDaemon({ dataDir: dir });
    const client = new DaemonClient({ socketPath: unixDaemon.socketPath! });
    ...
    await waitForStatus(unixDaemon.socketPath!, sub.run_id, "success");
    ...
    // ApiError on a 404 — the status the UI will rely on
    await expect(client.getRun("ghost")).rejects.toMatchObject({ name: "ApiError", status: 404 });

    // ── http transport (explicit baseUrl) ──
    httpDaemon = startDaemon({ dataDir: tmpDataDir("contract-client-http"), listen: { port: 0 } });
    expect(httpDaemon.baseUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
    const httpClient = new DaemonClient({ baseUrl: httpDaemon.baseUrl });
    expect((await httpClient.health()).ok).toBe(true);
    expect((await httpClient.listRuns()).runs).toEqual([]);
    await expect(httpClient.getRun("ghost")).rejects.toMatchObject({ name: "ApiError", status: 404 });

    // ── http transport via the SHOWRUNNER_DAEMON_URL env override ──
    process.env.SHOWRUNNER_DAEMON_URL = httpDaemon.baseUrl;
    const envClient = new DaemonClient(); // no explicit transport options
    expect((await envClient.health()).ok).toBe(true);
  } finally {
    if (savedUrl === undefined) delete process.env.SHOWRUNNER_DAEMON_URL;
    else process.env.SHOWRUNNER_DAEMON_URL = savedUrl;
    await unixDaemon?.close();
    await httpDaemon?.close();
    cleanupDir(dir);
    rmSync(cwd, { recursive: true, force: true });
  }
}, { timeout: 40_000 });
