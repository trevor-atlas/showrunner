...
import { daemonEntryPath } from "../../src/daemon.ts";
import { DaemonClient } from "../../src/client.ts";
import type { RunDetail } from "../../src/client.ts";
import { backfillMissedEvents, openDb } from "../../src/index.ts";
...
const POLL_TOOL = join(REPO_ROOT, "packages", "starter-kit", "src", "tools", "poll.ts");
...
function bootDaemon(dataDir: string, extra: Record<string, string> = {}): { child: ChildProcess; socketPath: string; logPath: string } {
  ...
  child.unref();
  closeSync(logFd);
  return { child, socketPath, logPath };
}
