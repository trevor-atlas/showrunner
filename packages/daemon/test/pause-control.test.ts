...
import { z } from "zod";
import { EnvelopeBase, defineAgent, defineBlueprint, socketPathFor } from "@showrunner/core";
...
import type { BlueprintRun, ScriptMap, ScriptedTurn } from "../src/index.ts";
...
async function waitForStatus(socketPath: string, runId: string, status: string, timeoutMs = 15_000): Promise<void> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
...
test("HTTP: approve + the pause viewer + steer land on a require_approval pause", async () => {
  ...
  try {
    ...
    const steered = await api(socketPath, "POST", `/runs/${runId}/steer`, { message: "be careful", by: "cli" });
    expect(steered.status).toBe(200);
    expect((steered.json as { queued_steers: number }).queued_steers).toBe(1);

    // approve → the run proceeds to spawn and succeeds
    const approved = await api(socketPath, "POST", `/runs/${runId}/approve`, { by: "cli" });
    expect(approved.status).toBe(200);
    await waitForStatus(socketPath, runId, "success");

    const detail = (await api(socketPath, "GET", `/runs/${runId}`)).json as { event_count: number };
    expect(detail.event_count).toBeGreaterThan(0);
    const events = (await api(socketPath, "GET", `/runs/${runId}/events?cursor=0&limit=500`)).json as {
...
test("F1 (server): a paused run blocks the next queued spawn while holding the slot (§5.4)", async () => {
  ...
  try {
    ...
    const a = await api(socketPath, "POST", "/runs", { blueprint: pauseBp, cwd: runCwd, delayMs: 0 });
    ...
    const aDone = (await api(socketPath, "GET", `/runs/${aId}`)).json as {
...
test(
  ...
  try {    // a SUBPROCESS daemon so we can actually kill it (in-process would kill the test)
    ...
    const happyBp = join(fixturesDir, "happy-blueprint.ts");
    const sub = await api(socketPath, "POST", "/runs", { blueprint: happyBp, cwd: runCwd, delayMs: 60 });
    const runId = (sub.json as { run_id: string }).run_id;
    ...
    // SIGKILL leaves the unix socket FILE behind (only a graceful close
    // removes it) — wait for the listener to go DOWN, then the restart
    // unlinks the stale socket itself (startDaemon)
    await waitFor(async () => {
      try {
        await api(socketPath, "GET", "/health");
        return false;
