...
import { socketPathFor } from "@showrunner/core";

import { cleanupDir, tmpDataDir } from "./helpers.ts";
import { daemonEntryPath } from "../src/index.ts";
...
function bootDaemon(label: string): BootedDaemon {
  ...
  closeSync(logFd);
  child.unref();
  return { child, dir, cwd, socketPath };
}

async function waitForHealth(socketPath: string, timeoutMs = 15_000): Promise<void> {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    try {
      const h = await api(socketPath, "GET", "/health", undefined, 1500);
      if (h.status === 200) return;
    } catch {
      // daemon not up yet (or mid-freeze)
    }
    if (Date.now() > deadline) throw new Error(`daemon not healthy at ${socketPath}`);
...
async function waitForStatus(socketPath: string, runId: string, status: string, timeoutMs = 20_000): Promise<void> {
  ...
  for (;;) {
    ...
    try {
      d = await api(socketPath, "GET", `/runs/${runId}`, undefined, 1500);
    } catch {
...
test("§19/F1: HTTP resolves DURING a sleeping gate (2.5s) — the event loop never blocks on gate execution", async () => {
  ...
  try {
    await waitForHealth(h.socketPath);
    const sub = await api(h.socketPath, "POST", "/runs", { blueprint: SLEEP_GATE_BP, cwd: h.cwd });
    ...
    const t0 = Date.now();
    const health = await api(h.socketPath, "GET", "/health", undefined, 1500);
    ...
    const t1 = Date.now();
    const runs = await api(h.socketPath, "GET", "/runs", undefined, 1500);
    const runsElapsed = Date.now() - t1;
    expect(runs.status).toBe(200);
    expect(runsElapsed).toBeLessThan(1500);

    await waitForStatus(h.socketPath, runId, "success", 20_000);
  } finally {
    teardown(h);
  }
}, { timeout: 30_000 });
...
test("§5.5: a gate that exceeds its cap → violation with the error text; the daemon stays healthy after", async () => {
  ...
  try {
    await waitForHealth(h.socketPath);
    const sub = await api(h.socketPath, "POST", "/runs", { blueprint: TIMEOUT_GATE_BP, cwd: h.cwd });
    expect(sub.status).toBe(201);
    const runId = (sub.json as { run_id: string }).run_id;

    // the capped gate always fails → the phase exhausts its budget → pause
    await waitForStatus(h.socketPath, runId, "paused", 20_000);
    const p = await api(h.socketPath, "GET", `/runs/${runId}/pause`);
    expect((p.json as any).kind).toBe("budget_exhausted");

    // every gate_result row recorded the cap failure with the error text
    const gates = await api(h.socketPath, "GET", `/runs/${runId}/phases/build/gates`);
    ...
    const health = await api(h.socketPath, "GET", "/health", undefined, 3000);
    expect(health.status).toBe(200);
    expect(health.json).toEqual({ ok: true });

    // the run is still controllable — fail it to terminal
    const fail = await api(h.socketPath, "POST", `/runs/${runId}/fail`, { by: "test" });
    expect(fail.status).toBe(200);
    await waitForStatus(h.socketPath, runId, "failed", 10_000);
  } finally {
    teardown(h);
  }
}, { timeout: 30_000 });
